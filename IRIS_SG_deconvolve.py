...
def FFT_conv_1D(datain, psfin, div = False, rev_psf=False):
    ...
    if ydiff <= 0:
        ...
        if np.abs(ydiff) % 2 == 1:
            pin = psfin[int(rs+1):int(psflen-rs)] # odd ydiff
        else:
            pin =  psfin[int(rs):int(psflen-rs)] # even ydiff

       # renormalize PSF
        pin = pin/np.sum(pin) 
        
    # Pad the PSF if it is too long
    if ydiff > 0:
        
        rs = ydiff/2
        
        padl = np.zeros(int(rs),dtype=float)
        
        if ydiff % 2 == 1:
            padr = np.zeros(int(rs+1),dtype=float)
        else:
            padr = np.zeros(int(rs),dtype=float)
